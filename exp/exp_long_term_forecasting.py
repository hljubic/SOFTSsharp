...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _select_criterion(self):
        criterion = nn.MSELoss()#AsymmetricLogWeightedMAE()#WeightedMSE()
        return criterion
    ...
    def test(self, setting, test=0, csv_data = None):
        test_data, test_loader = self._get_data(flag='test')
        if test:
            print('loading model')
        ...
        if csv_data is not None:
            ...
            # Define the CSV file path with the date included
            csv_file = os.path.join(results_dir, f'results_{today}.csv')
            ...
            # Add values from setting_data_dict to experiment_data
            experiment_data.update(csv_data)
